...
class InsFinancialReportXlsx(models.AbstractModel):
    _name = "report.plant_accounting_report.ins_financial_xlsx_aa_account"
    _description = "Report Dynamic and Financial Report"
        ...
        if filter:
            ...
            if filter["form"].get("date_from"):
                self.sheet_2.write_datetime(
                    self.row_pos_2, 1, date, self.content_header_date
                )
            self.row_pos_2 += 1
            # Date to
            self.sheet_2.write_string(
            ...
            if filter["form"]["enable_filter"]:
                ...
                self.sheet_2.write_string(
                    self.row_pos_2, 0, _("Comparison Date from"), self.format_header
                )
                date = self.convert_to_date(
                ...
                # Compariosn Date to
                self.sheet_2.write_string(
                    self.row_pos_2, 0, _("Comparison Date to"), self.format_header
                )
                date = self.convert_to_date(
    ...
    def prepare_report_contents(self, data):
        ...
        if data["form"]["debit_credit"] == 1:
            ...
            new_start = 1
            new_stop = 3
            for title in data["analytic_data"].values():
                ...
                new_stop = new_start + 2
            self.row_pos += 1

            self.sheet.write_string(self.row_pos, 0, _("Name"), self.format_header)
            new_start = 1
            for analytic in data["analytic_data"]:
                self.sheet.write_string(
                    self.row_pos, new_start, _("Debit"), self.format_header
                )
                new_start += 1
                self.sheet.write_string(
                    self.row_pos, new_start, _("Credit"), self.format_header
                )
                new_start += 1
                self.sheet.write_string(
                    self.row_pos, new_start, _("Balance"), self.format_header
                )
                new_start += 1
            ...
            for a in data["report_lines"]:
                ...
                new_start = 1
                for key in data["dr_cr_bal_dict_keys"]:
                    self.sheet.write_number(
                        self.row_pos, new_start, float(a.get(key, 0.0)), tmp_style_num
                    )
                    new_start += 1
        ...
        if data["form"]["debit_credit"] != 1:
            ...
            if data["form"]["enable_filter"]:
                self.sheet.write_string(
                    self.row_pos, 1, data["form"]["label_filter"], self.format_header
                )
                self.sheet.write_string(
                    self.row_pos, 2, _("Balance"), self.format_header
                )
            else:
                self.sheet.write_string(
                    self.row_pos, 1, _("Balance"), self.format_header
                )
            ...
            for a in data["report_lines"]:
                ...
                if data["form"]["enable_filter"]:
                    self.sheet.write_number(
                        self.row_pos, 1, float(a.get("balance_cmp")), tmp_style_num
                    )
                    self.sheet.write_number(
                        self.row_pos, 2, float(a.get("balance")), tmp_style_num
                    )
                else:
                    self.sheet.write_number(
                        self.row_pos, 1, float(a.get("balance")), tmp_style_num
                    )
        if (
            ...
            self.row_pos += 2
            self.sheet.merge_range(
                self.row_pos, 1, self.row_pos, 2, "Initial Cash Balance", tmp_style_num
            )
            self.sheet.write_number(
                self.row_pos, 3, float(data.get("initial_balance")), tmp_style_num
            )
            self.row_pos += 1
            self.sheet.merge_range(
                self.row_pos, 1, self.row_pos, 2, "Current Cash Balance", tmp_style_num
            )
            self.sheet.write_number(
                self.row_pos, 3, float(data.get("current_balance")), tmp_style_num
            )
            self.row_pos += 1
            self.sheet.merge_range(
                self.row_pos, 1, self.row_pos, 2, "Net Cash Balance", tmp_style_num
            )
            self.sheet.write_number(
                self.row_pos, 3, float(data.get("ending_balance")), tmp_style_num
            )
