...
class InsFinancialReport(models.TransientModel):
    ...
    def _compute_account_balance(self, accounts, report):
        ...
        mapping = {}
        if not self.analytic_ids:
            self.analytic_ids = [(6, 0, self._get_analytic_account().ids)]
        for analytic_acc_id in self.analytic_ids.sorted(lambda x: x.id):
            mapping.update(
                {
                    "%s-debit" % analytic_acc_id.id: "COALESCE(SUM(debit), 0) as debit",
                    "%s-credit"
                    % analytic_acc_id.id: "COALESCE(SUM(credit), 0) as credit",
                    "%s-balance"
                    % analytic_acc_id.id: "COALESCE(SUM(debit),0) - COALESCE(SUM(credit), 0) as balance",
        ...
        if (
            self.env.context.get("date_from", False)
            and self.env.context.get("date_from", False) != date_from
        ):
            date_from = self.date_from_cmp or False
        if (
            self.env.context.get("date_to", False)
            and self.env.context.get("date_to", False) != date_to
        ):
        ...
        for account in accounts:
            for analytic_acc_id in self.analytic_ids.sorted(lambda x: x.id):
                new_res[
                    str(account.id) + "_" + str(analytic_acc_id.id)
                ] = dict.fromkeys(mapping, 0.0)
        if accounts:
            if (
                self.account_report_id
                != self.env.ref(
                ...
                # Validation
                if (
                    report.type in ["accounts", "account_type"]
                    and not report.range_selection
                ):
                    raise UserError(
                        _('Please choose "Custom Date Range" for the report head %s')
                        % (report.name)
                    )

                if (
                    report.type in ["accounts", "account_type"]
                    and report.range_selection == "from_the_beginning"
                ):
                    context.update({"strict_range": False})
                # For equity
                if (
                    report.type in ["accounts", "account_type"]
                    and report.range_selection == "current_date_range"
                ):
                    if date_to and date_from:
                        ...
                        raise UserError(
                            _(
                                "From date and To date are mandatory to generate this report"
                            )
                        )
                if (
                    report.type in ["accounts", "account_type"]
                    and report.range_selection == "initial_date_range"
                ):
                    if date_from:
                        context.update(
                            {
            ...
            request = (
                "SELECT account_id as id , analytic_account_id as analytic_account_id ,"
                + ", ".join(mapping.values())
                + " FROM "
                + tables
            ...
            for row in self.env.cr.dictfetchall():
                aa_debit_key = str(row.get("analytic_account_id")) + "-debit"
                aa_credit_key = str(row.get("analytic_account_id")) + "-credit"
                aa_balance_key = str(row.get("analytic_account_id")) + "-balance"
                row_key = str(row["id"]) + "_" + str(row["analytic_account_id"])
                row["id"] = row_key
                new_res[row["id"]] = row
                new_res[row["id"]][aa_debit_key] = row.get("debit")
                new_res[row["id"]][aa_credit_key] = row.get("credit")
                new_res[row["id"]][aa_balance_key] = row.get("balance")
    ...
    def _compute_report_balance(self, reports):
        ...
        fields = []
        for analytic_acc in self.analytic_ids.sorted(lambda x: x.id):
            fields += [
                "%s-debit" % analytic_acc.id,
                "%s-credit" % analytic_acc.id,
                "%s-balance" % analytic_acc.id,
            ]
    ...
    def get_account_lines(self, data):
        ...
        for report in child_reports:
            ...
            total_balance = 0
            for analytic_acc_id in self.analytic_ids.sorted(lambda x: x.id):
                analytic_balance = "%s-balance" % analytic_acc_id.id
                analytic_debit = "%s-debit" % analytic_acc_id.id
                analytic_credit = "%s-credit" % analytic_acc_id.id
                if res[report.id]:
                    vals.update(
                        {
                            analytic_balance: res[report.id].get(analytic_balance, 0.0)
                            * int(report.sign)
                        }
                    )
                    total_balance += res[report.id].get(analytic_balance, 0.0)
                    if data["debit_credit"]:
                        vals[analytic_debit] = res[report.id].get(analytic_debit, 0.0)
                        vals[analytic_credit] = res[report.id].get(analytic_credit, 0.0)
                        total_debit += res[report.id].get(analytic_debit, 0.0)
                        total_credit += res[report.id].get(analytic_credit, 0.0)
            vals["total_credit"] = total_credit
            vals["total_debit"] = total_debit
            vals["total_balance"] = total_balance
            if data["enable_filter"]:
            ...
            if res[report.id].get("account"):
                ...
                for account_id, value in res[report.id]["account"].items():
                    flag = False
                    new_account = account_id.split("_")
                    new_account = new_account and new_account[0] or None
                    account = self.env["account.account"].browse(int(new_account))
                    vals = {
                        "account": account.id,
                    ...
                    for analytic_acc_id in self.analytic_ids.sorted(lambda x: x.id):
                        ...
                        analytic_debit = "%s-debit" % analytic_acc_id.id
                        analytic_credit = "%s-credit" % analytic_acc_id.id
                        if value.get(analytic_balance):
                            vals.update(
                                {
                                    analytic_balance: value.get(analytic_balance, 0.0)
                                    * int(report.sign)
                                    or 0.0
                                }
                            )
                            total_balance += value.get(analytic_balance)
                        if data["debit_credit"]:
                            vals[analytic_debit] = value.get(analytic_debit, 0.0)
                            vals[analytic_credit] = value.get(analytic_credit, 0.0)
                            total_debit += value.get(analytic_debit, 0.0)
                            total_credit += value.get(analytic_credit, 0.0)
                            if not currency_id.is_zero(
                                vals[analytic_debit]
                            ) or not currency_id.is_zero(vals[analytic_credit]):
                                flag = True
                    vals["total_credit"] = total_credit
                    vals["total_debit"] = total_debit
                    vals["total_balance"] = total_balance
    ...
    def get_report_values(self):
        ...
        used_context["analytic_account_ids"] = self.analytic_ids.sorted(lambda x: x.id)
        ...
        comparison_context["analytic_account_ids"] = self.analytic_ids.sorted(
            lambda x: x.id
        )
        ...
        (
            ...
            current_balance,
            ending_balance,
        ) = self.get_account_lines(data.get("form"))
        ...
        elif self.account_report_id == self.env.ref(
        ...
        for analytic_acc_id in self.analytic_ids.sorted(lambda x: x.id):
            analytic_dict_name_data.update({analytic_acc_id.id: analytic_acc_id.name})
        analytic_dict_name_data.update({999999999999999999999999999999: "Total"})
        data["analytic_data"] = analytic_dict_name_data
        dr_cr_bal_dict = {}
        dr_cr_bal_keys = []

        for dr_cr_bal in self.analytic_ids.sorted(lambda x: x.id):
            dr_cr_bal_dict.update(
                {
                    str(dr_cr_bal.id) + "-debit": 0,
                    str(dr_cr_bal.id) + "-credit": 0,
                    str(dr_cr_bal.id) + "-balance": 0,
                }
            )
            dr_cr_bal_keys += [
                str(dr_cr_bal.id) + "-debit",
                str(dr_cr_bal.id) + "-credit",
                str(dr_cr_bal.id) + "-balance",
            ]
        dr_cr_bal_keys += [
            "total_debit",
            "total_credit",
        ...
        dr_cr_bal_dict.update({"total_credit": 0, "total_debit": 0, "total_balance": 0})
        data["dr_cr_bal_dict"] = dr_cr_bal_dict
        data["dr_cr_bal_dict_keys"] = dr_cr_bal_keys

        print("////////////////", dr_cr_bal_keys, "-----=============", data)
        return data
    ...
    def action_xlsx(self):
        ...
        return self.env.ref(
            "plant_accounting_report.action_ins_financial_report_aa_xlsx"
        ).report_action(self)
